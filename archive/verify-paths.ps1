...
# Verifies that all paths in HTML files use the /archive/ prefix
...
foreach ($file in $htmlFiles) {
    ...
    $relativePath = $file.FullName.Replace($archiveDir, "").TrimStart("\")
...
if (Test-Path $robotsPath) {
    ...
    if ($robotsContent -notmatch 'Sitemap:.*archive') {
...
foreach ($file in $criticalFiles) {
    $path = Join-Path $archiveDir $file
...
Write-Host "`nTotal issues: $($errors.Count + $warnings.Count)" -ForegroundColor $(if ($errors.Count -gt 0) { "Red" } else { "Yellow" })
