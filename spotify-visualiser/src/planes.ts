...
export default class Planes {
  ...
  async fetchCovers() {
    // Only use local images hosted with the project; no external APIs
    const BASE_URL = import.meta.env.BASE_URL || "/spotify-visualiser/"
    const urls: string[] = new Array(30)
      .fill(0)
      .map((_, i) => `${BASE_URL}covers/image_${i}.jpg`)
  ...
  createMaterial() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uWrapperTexture: {
          value: new THREE.TextureLoader().load(`${BASE_URL}spt-3.png`, (tex) => {
